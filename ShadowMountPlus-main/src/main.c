...
int main(void) {
  ...
  log_debug(
      "Sonic-Loader exFAT/UFS/PFS/LVD/MD. "
      "FW: %s. "
      "Build: %s %s. "
  ...
  notify_system("Sonic-Loader exFAT/UFS/PFS");
  log_non_empty_scan_paths();

  if (runtime_config()->legacy_recursive_scan_forced) {
    notify_system_info("Sonic-Loader: recursive_scan=1 deprecated, using scan_depth=2.");
  } else if (runtime_config()->scan_depth > 1u) {
    notify_system_info("Sonic-Loader: scan depth %u enabled.",

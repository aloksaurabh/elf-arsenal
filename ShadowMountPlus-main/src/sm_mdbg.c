...
static void handle_pre_pause_failure(const char *reason) {
  ...
  notify_system_info("Sonic-Loader: app crashed before kstuff pause: %s. kstuff is not to "
                     "blame.",
...
static void handle_post_pause_failure(const char *reason, uint64_t now_us) {
  ...
  if (post_pause_us > MDBG_AUTOTUNE_WINDOW_US) {
    ...
    notify_system_info("Sonic-Loader: app crashed after kstuff pause: %s. autotune skipped.",
                       g_mdbg.game.title_id);
    clear_tracked_game();
    return;
  }
  ...
  if (upsert_kstuff_autotune_pause_delay(g_mdbg.game.title_id,
    ...
    if (is_rtld_error) {
      ...
      notify_system_info("Sonic-Loader: crash after kstuff pause, delay raised for %s "
  ...
  log_debug("  [MDBG] failed to persist autotune pause delay for %s%s%s",
            g_mdbg.game.title_id, reason ? ": " : "", reason ? reason : "");
  notify_system_info("Sonic-Loader: crash after kstuff pause: %s. failed to update "
                     "autotune delay.",
                     g_mdbg.game.title_id);
  clear_tracked_game();
}

...
/* Sonic Loader: dedupe notifications. The same message text within
   NOTIFY_DEDUPE_SECONDS is suppressed (only logged) so the user doesn't
   see toasts repeating during scan loops, installs, etc. */
#define NOTIFY_DEDUPE_SECONDS 60
static char     g_last_notify_msg[1024];
static time_t   g_last_notify_at = 0;
...
static void notify_system_plain_message(const char *message) {
  ...
  bool dedupe = (g_last_notify_at != 0 &&
                 (now - g_last_notify_at) < NOTIFY_DEDUPE_SECONDS &&
                 strcmp(g_last_notify_msg, req.message) == 0);
  if (!dedupe) {
    (void)strlcpy(g_last_notify_msg, req.message, sizeof(g_last_notify_msg));
    g_last_notify_at = now;
  }
  ...
  log_debug("NOTIFY%s: %s", dedupe ? "[dedup]" : "", req.message);
}
...
void notify_system_rich(bool allow_in_quiet_mode, const char *fmt, ...) {
  ...
  /* Dedupe rich toasts the same way as plain ones. */
  time_t now = time(NULL);
  pthread_mutex_lock(&g_log_mutex);
  bool dedupe = (g_last_notify_at != 0 &&
                 (now - g_last_notify_at) < NOTIFY_DEDUPE_SECONDS &&
                 strcmp(g_last_notify_msg, message) == 0);
  if (!dedupe) {
    (void)strlcpy(g_last_notify_msg, message, sizeof(g_last_notify_msg));
    g_last_notify_at = now;
  }
  pthread_mutex_unlock(&g_log_mutex);

  if (dedupe) {
    log_debug("NOTIFY[dedup]: %s", message);
    return;
  }

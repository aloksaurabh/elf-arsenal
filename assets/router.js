...
/* Elf Arsenal hosts this Homebrew launcher under /homebrew, not /
   (which serves the title launcher). Centralise the home path so all
   the in-app history/URL writes stay scoped under /homebrew. */
const HOME_PATH = "/homebrew";
...
class Router {
    ...
    async handlePopState(event) {
        ...
        try {
            ...
            if (currentContent) {
                ...
                if (currentContent.getAttribute("data-router-path") === encodeURIComponent(HOME_PATH)) {
    ...
    async handleHome() {
        await this.pushOrReplaceState(HOME_PATH);
        await renderHomePage();
    }

    async handleFallbackHome() {
        let homeContent = /** @type {HTMLElement?} */ (document.querySelector(`[data-router-path="${encodeURIComponent(HOME_PATH)}"]`));
        // delete current content
        const currentContent = document.getElementById("content");
        ...
        if (!homeContent) {
            ...
            homeContent.setAttribute("data-router-path", encodeURIComponent(HOME_PATH));
        ...
        history.replaceState({ path: HOME_PATH, dataRouterId: contentUuid }, "", HOME_PATH);
    }

...
// @ts-ignore
const baseURL = window.location.origin == "null" ? "http://127.0.0.1:6969" : window.location.origin;
...
class ApiClient {
    ...
    static async launchApp(path, args = null, env = null, cwd = null, daemon = false) {
        ...
        if (!response.ok) {
            /* Elf Arsenal >= 1.0.36 returns a JSON body on 503 with the
               specific failing stage from hbldr.c (e.g. "no signed-in
               user"). Read it if present so the user sees an actionable
               error instead of "status code: 503". */
            let detail = "";
            try {
            ...
            throw new Error("Failed to launch app, status code: " +
